...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int buff_size = 2;
	int byte_count = 2;
	BYTE write_buffer[buff_size];
	BYTE read_buffer[buff_size];

	// select
	XSpi_SetSlaveSelect(&SpiInstance, mask);

	// utilize xspi_transfer function for writing
	write_buffer[0] = reg + 2;
	write_buffer[1] = val;

	int indicator = XSpi_Transfer(&SpiInstance, write_buffer, read_buffer, byte_count);

	if (indicator != 0) {
		xil_printf ("Failed to write the bytes sent!");
	}
	XSpi_SetSlaveSelect(&SpiInstance, mask_off);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for (i = 0; i < nbytes; i++) {
		write_buffer[0] = reg + 2;
		write_buffer[1] = data[i]; // *(data+i)
		indicator = XSpi_Transfer(&SpiInstance, write_buffer, read_buffer, buff_size);
		if (indicator != 0) {
				xil_printf ("Failed to write the bytes sent!");
			}
	}
...
//	XSpi_Transfer(&SpiInstance, write_buffer, NULL, 1);

	int indicator = XSpi_Transfer(&SpiInstance, write_buffer, read_buffer, byte_count);

	val = read_buffer[1]; //won't val be read from 2nd index

	if (indicator != 0) {
		xil_printf ("Failed to read the bytes sent!");
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	// utilize xspi_transfer function for reading
