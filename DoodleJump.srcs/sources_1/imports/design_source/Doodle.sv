...
( 
    ...
    always_comb begin
        ...
        if (keycode == 8'h04) begin
            ...
            Ball_Y_Motion_next = 10'd1; // should be a certain constant to implement gravity
        end
        
        // should not need S keycode 
...
//      TASK: decide either to make an if condition or utilize else if within the for loop 
        ...
        else if ( (BallY - BallS_Y) >= Ball_Y_Max)  // Ball is at the bottom edge, game over!
        begin
...
//              Ball_X_Motion_next = (~(BallS) + 1'b1) + (~(BallX) + 1'b1) + Ball_X_Min; // keep the same way but start the ball at the left
              ...
              Ball_Y_Motion_next = Ball_Y_Step; // gravity accounted
        end
...
//              Ball_X_Motion_next = (~(BallX) + 1'b1) + BallS + Ball_X_Max; // go to furthest right 
    ...
    begin
        ...
        begin
            ...
            Ball_Y_next = BallY + Ball_Y_Motion_next; // gravity accounted 
        end
        // if left edge
    ...
    begin: Move_Ball
        ...
        begin 
            Ball_Y_Motion <= 10'd1; //Ball_Y_Step, set to 1 initially to abstract gravity logic 
